...
const Sidebar: React.FC = () => {
  ...
  const userLinks = [
    { path: '/dashboard/user', label: 'My Dashboard' },
    { path: '/bookings', label: 'My Bookings' },
  ];

  const professionalLinks = [
    { path: '/dashboard/professional', label: 'Dashboard' },
    { path: '/schedule', label: 'Schedule' },
    { path: '/clients', label: 'Clients' },
  ];

  const links = user?.role === 'professional' ? professionalLinks : userLinks;

  return (
    <motion.div 
      className="w-64 bg-muted min-h-screen p-4 shadow-lg"
      initial={{ x: -300 }}
      animate={{ x: 0 }}
